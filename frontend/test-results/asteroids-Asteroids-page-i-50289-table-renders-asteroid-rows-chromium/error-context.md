...
- Name: asteroids.spec.ts >> Asteroids page >> inspection table renders asteroid rows
- Location: e2e\asteroids.spec.ts:135:3
...
Locator: getByText('Inspection table')
Expected: visible
Error: strict mode violation: getByText('Inspection table') resolved to 2 elements:
    1) <p class="mt-4 max-w-3xl text-base leading-7 text-[var(--color-text-faint)]">The backend constrains the date range to NASA's s…</p> aka getByText('The backend constrains the')
    2) <p>Inspection table</p> aka getByText('Inspection table', { exact: true })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Inspection table')
...
```ts
  46  |     await page.goto('/asteroids');
  47  |     await expect(page.getByRole('heading', { level: 1, name: 'Asteroids' })).toBeVisible();
  48  |   });
  49  | 
  50  |   test('section heading and description are visible', async ({ page }) => {
  51  |     await page.goto('/asteroids');
  52  |     await expect(page.getByText('Asteroid Analytics')).toBeVisible();
  53  |     await expect(
  54  |       page.getByRole('heading', { level: 2, name: /Inspect velocity, scale, and risk/ })
  55  |     ).toBeVisible();
  56  |   });
  57  | 
  58  |   test('start and end date navigation controls are present', async ({ page }) => {
  59  |     await page.goto('/asteroids');
  60  | 
  61  |     await expect(page.getByText('Start date')).toBeVisible();
  62  |     await expect(page.getByText('End date')).toBeVisible();
  63  |     await expect(page.getByRole('button', { name: /Start date:/ })).toBeVisible();
  64  |     await expect(page.getByRole('button', { name: /End date:/ })).toBeVisible();
  65  |   });
  66  | 
